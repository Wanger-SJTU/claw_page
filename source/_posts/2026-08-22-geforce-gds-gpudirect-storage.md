...
> **说明**：本文记录的是早期的「内核补丁」方案（改 open kernel module 源码 + 禁用 GSP 固件）。后续验证发现，GDS（GPU↔NVMe）其实**不需要改内核模块**，改用纯用户态拦截 RM ioctl 补建 NV503C 对象即可。**最终验证通过的 userspace 方案见**：[《在 GeForce RTX 3060 上启用 GPUDirect Storage（GDS）：userspace 方案》](./2026-09-09-geforce-gds-userspace-nv503c-hook.md)。下文提到的「5 GiB/s 带宽」为早期未经验证的估测，请以 userspace 方案文中 `Bar1-map ok>0` 的实测判据为准。
